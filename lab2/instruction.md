...
overlap(Ball& b) should first check if the ball of the current instance collides with ball b. There are four conditions to check as overlaps can happen from 4 angles. If it doesn't collide, return that there is no overlap. If it collides with it, the function determines in which direction the collision is happening. The function returns an int, which is either HORIZONTAL_OVERLAP, VERTICAL_OVERLAP or NO_OVERLAP. The values of these macros are defined in Globals.h. If there is no overlap, the function should return NO_OVERLAP. In Fig. 4, we show how to measure the dimensions of the overlap. If the vertical overlap is larger than the horizontal, the function should return VERTICAL_OVERLAP. Otherwise, it should return HORIZONTAL_OVERLAP. (This function is optional to implement. We don't test for it. You can have it return NO_OVERLAP).
